      ...
      if (res.data.code == 200) {
        if (res.data.data.records.length == 0) {
          ...
          for (var i = 0; i < l.length; i++) {
            ...
            l[i].content2 = l[i].content ? l[i].content.length > 43 ? l[i].content.substring(0, 43) + '...' : l[i].content : ''
          }
...
Page({
  ...
  search: function () {
    wx.navigateTo({
      url: '../search/search?k=complaintSearch&p=请输入标题关键字查询',
    })
  },
  // 跳转到消息的详情页面
  goDetail: function (e) {
    console.log(e)
    wx.navigateTo({
      url: './complaintdetail?id=' + e.currentTarget.dataset.infoid,
    })
  },
  // 添加投诉
